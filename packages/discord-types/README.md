...
While it was primarily created for Dussicord based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @dussicord/discord-types
yarn add -D @dussicord/discord-types
pnpm add -D @dussicord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@dussicord/discord-types";
...
```ts
import { ApplicationCommandType } from "@dussicord/discord-types/enums";
