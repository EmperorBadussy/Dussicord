...
export const dussicordDevs = {} as Record<string, Dev>;
...
export function parseDussicordDevs() {
    const file = createSourceFile("constants.ts", readFileSync("src/utils/constants.ts", "utf8"), ScriptTarget.Latest);

    for (const child of file.getChildAt(0).getChildren()) {
        if (!isVariableStatement(child)) continue;

        const devsDeclaration = child.declarationList.declarations.find(d => hasName(d, "DussicordDevs"));
        if (!devsDeclaration?.initializer || !isCallExpression(devsDeclaration.initializer)) continue;

        const value = devsDeclaration.initializer.arguments[0];

        if (!isSatisfiesExpression(value) || !isObjectLiteralExpression(value.expression)) throw new Error("Failed to parse DussicordDevs: not an object literal");

        for (const prop of value.expression.properties) {
            const name = (prop.name as Identifier).text;
            const value = isPropertyAssignment(prop) ? prop.initializer : prop;

            if (!isObjectLiteralExpression(value)) throw new Error(`Failed to parse DussicordDevs: ${name} is not an object literal`);
    ...
    throw new Error("Could not find DussicordDevs constant");
}
            ...
            switch (key) {
                ...
                case "authors":
                    ...
                    data.authors = value.elements.map(e => {
                        ...
                        const d = devs[getName(e)!] || dussicordDevs[getName(e)!];
