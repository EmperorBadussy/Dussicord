...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("dussicord", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("dussicord://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Dussicord] Installed React Developer Tools"))
                    .catch(err => console.error("[Dussicord] Failed to install React Developer Tools", err));
        } catch { }
