...
export const DATA_DIR = process.env.DUSSICORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "DussicordData", suffix)
        : join(app.getPath("userData"), "..", "Dussicord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Dussicord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
