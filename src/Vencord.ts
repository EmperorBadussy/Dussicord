    ...
    if (!hasCloudAuth) {
        if (Settings.cloud.authenticated) {
            ...
            showNotification({
                ...
                onClick: () => SettingsRouter.openUserSettings("dussicord_cloud_panel")
            });
...
async function runUpdateCheck() {
    ...
    try {
        ...
        showNotice(
            "A new version of Dussicord is available!",
            "View Update",
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("An Dussicord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Dussicord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Dussicord.",
