    ...
    patches: [
        {
            ...
            replacement: [
                ...
                },
                {
                    ...
                    replace: (m, notice) => `if(${notice}?.id=="DussicordNotice")return(${notice}=null,Vencord.Api.Notices.nextNotice(),true);${m}`
                },
                // FIXME(Bundler agressive inline): Remove the non used compability once enough time has passed
                {
                    ...
                    replace: (m, _, notice) => `if(${notice}?.id=="DussicordNotice")return(${notice}=null,Vencord.Api.Notices.nextNotice(),true);${m}`,
                    noWarn: true
                }
            ]
        }
