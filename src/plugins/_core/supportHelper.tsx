...
import { platformName } from "@dussicordplugins/dussicordHelper/utils";
import customIdle from "@plugins/customIdle";
import { gitHash, gitHashShort } from "@shared/vencordUserAgent";
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, DUSSICORD_TEAM, GUILD_ID, SUPPORT_CHANNEL_ID, SUPPORT_CHANNEL_IDS, VC_CONTRIB_ROLE_ID, VC_DONOR_ROLE_ID, VC_GUILD_ID, VC_REGULAR_ROLE_ID, VENCORD_CONTRIB_ROLE_ID } from "@utils/constants";
import { sendMessage } from "@utils/discord";
import { Logger } from "@utils/Logger";
import { Margins } from "@utils/margins";
import { isAnyPluginDev, isDussicordGuild, isDussicordSupport, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    DUSSICORD_TEAM, // Dussicord Team
    DONOR_ROLE_ID, // Dussicord Donor
    CONTRIB_ROLE_ID, // Dussicord Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` • [${clientInfo.shortHash}](<https://github.com/Dussicord/Equibop/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const info = {
        Dussicord:
            `v${VERSION} • [${gitHashShort}](<https://github.com/EmperorBadussy/Dussicord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Dussicord DevBuild": !IS_STANDALONE,
...
export default definePlugin({
    ...
    commands: [
        {
            name: "dussicord-debug",
            description: "Send Dussicord debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isDussicordGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "dussicord-plugins",
            description: "Send Dussicord plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isDussicordGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    return Alerts.show({
                        title: "Hold on!",
                        body: <div>
                            <Paragraph>You are using an outdated version of Dussicord! Chances are, your issue is already fixed.</Paragraph>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using an externally updated Dussicord version, the ability to help you here may be limited.</Paragraph>
                        <Paragraph className={Margins.top8}>
                            Please join the <Link href="https://equicord.org/discord">Dussicord Server</Link> for support,
            ...
            if (!IS_STANDALONE && !settings.store.dismissedDevBuildWarning) {
                return Alerts.show({
                    title: "Hold on!",
                    body: <div>
                        <Paragraph>You are using a custom build of Dussicord, which we do not provide support for!</Paragraph>

                        <Paragraph className={Margins.top8}>
                            We only provide support for <Link href="https://github.com/EmperorBadussy/Dussicord">official builds</Link>.
                            Either <Link href="https://github.com/Dussicord/Equilotl">switch to an official build</Link> or figure your issue out yourself.
                        </Paragraph>
    ...
    renderMessageAccessory(props) {
        ...
        const dussicordSupport = isDussicordSupport(props.message.author.id);

        const shouldAddUpdateButton =
            !IS_UPDATER_DISABLED
            && ((isSupportChannel(props.channel.id) && dussicordSupport))
        ...
        if (isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel) && dussicordSupport) {
            if (props.message.content.includes("/dussicord-debug") || props.message.content.includes("/dussicord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Dussicord & Vencord plugin developers for support!
                <br />
