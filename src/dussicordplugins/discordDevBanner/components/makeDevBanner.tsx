...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, DussicordIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {dussicordIcon} {dussicordName} {dussicordVersion} ({dussicordHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{dussicordVersion}/g, VERSION)
        .replace(/{dussicordHash}/g, gitHashShort)
        .replace(/{dussicordPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{dussicordIcon}":
                return <span key={`icon-dussicord-${i}`} className="vc-discord-dev-banner-icons"><DussicordIcon /></span>;
            case "{electronIcon}":
