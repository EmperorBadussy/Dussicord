...
import { isDussicordGuild, isDussicordSupport } from "@utils/misc";
...
export const PluginButtons = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    const isDussicord = isDussicordGuild(message.channel_id) && isDussicordSupport(message.author.id);
    const startsWithEnabled = msg.startsWith("enable");
    const startsWithDisabled = msg.startsWith("disable");

    const shouldAddPluginButtons = pluginData && isDussicord && (startsWithEnabled || startsWithDisabled);
            ...
            <Button
                ...
                onClick={async () => {
                    try {
                        ...
                        new Logger("DussicordHelper").error("Error while toggling:", e);
