...
import { isDussicordPluginDev, isPluginDev } from "@utils/misc";
...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "DussicordDonor":
            return (
                <span style={{ order: settings.store.DussicordDonorPosition }}>
                    {badges.getDussicordDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
                            className={roleIcon}
        ...
        case "DussicordContributer":
            return isDussicordPluginDev(author.id) ? (
                <span style={{ order: settings.store.DussicordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="Dussicord Contributor"
                        size={20}
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showDussicordDonor && <CheckBadge badge={"DussicordDonor"} author={author} />}
            {settings.store.showDussicordContributor && <CheckBadge badge={"DussicordContributer"} author={author} />}
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, DussicordDevs.KrystalSkull],
